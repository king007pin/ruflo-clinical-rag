...
function AgentCard({ agent, meta, color, idx, phase, pending, compact }: {
  ...
  const role = meta?.role ?? agent?.reasoning ?? "";
  return (
    <div className="rounded-xl border flex flex-col transition-all duration-700"
        ...
        <div className="flex items-center gap-1.5">
          {agent ? (
            <>
              ...
              <span className="text-[10px] rounded-full px-2 py-0.5"
                style={{ backgroundColor: color.bg, color: "var(--muted)" }}>{role}</span>
            </>
          ) : (
            <span className="text-[10px]" style={{ color: "var(--muted)" }}>reasoning<ThinkingDots /></span>
          )}
        </div>
      </div>
      <div className="px-3 py-2.5 flex-1">
        {agent ? (
          <p className="text-xs leading-relaxed whitespace-pre-wrap"
            style={{ color: "var(--muted)", maxHeight: compact ? "8rem" : "14rem", overflowY: "auto" }}>
            {agent.message}
          </p>
        ) : <SkeletonLines />}
      </div>
      {agent && (
        <div className="px-3 py-1.5 border-t flex items-center gap-1" style={{ borderColor: color.border }}>
          ...
          <span className="text-[10px]" style={{ color: "var(--muted)" }}>{agent.reasoning}</span>
        </div>
      )}
    </div>
  );
