...
const DELAY_MS = 1000;
const BASE = "https://goldcopd.org";
const UA = "MediqRAG/1.0 (clinical research; contact: admin@mediq.ai)";


const INDEX_PAGES = [
  `${BASE}/gold-reports/`,
  `${BASE}/guidelines-global-strategy-for-asthma-management/`,
  `${BASE}/pocket-guide-to-copd-diagnosis-management-and-prevention/`,
];
...
async function extractLinks(pageUrl: string): Promise<string[]> {
  try {
    ...
    const links: string[] = [];
    for (const m of html.matchAll(/href="(https?:\/\/goldcopd\.org\/[^"]+)"/gi)) {
      if (!seen.has(m[1])) { seen.add(m[1]); links.push(m[1]); }
    }
    for (const m of html.matchAll(/href="(\/[^"]+)"/gi)) {
      const full = `${BASE}${m[1]}`;
      if (!seen.has(full) && (full.includes("report") || full.includes("guideline") || full.includes("gold") || full.includes("copd"))) {
...
export const goldCopdCrawler: CrawlerDef = {
  id: "gold-copd",
  name: "GOLD — COPD Guidelines",
  description: "GOLD COPD annual report — COPD diagnosis, staging (GOLD 1–4), pharmacological and non-pharmacological management guidelines",
  ...
  async fetchArticle(url: string): Promise<CrawlerArticle | null> {
    try {
      ...
      const title = (h1Match?.[1] ?? titleMatch?.[1] ?? "GOLD COPD Report").replace(/&[a-z]+;/g, " ").trim();
      const main = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i)?.[1]
        ?? html.match(/<div[^>]+class="[^"]*entry-content[^"]*"[^>]*>([\s\S]*?)<\/div>/i)?.[1]
        ?? html;
      const content = stripHtml(main);
      if (content.length < 150) return null;
      return { url, title: title.slice(0, 200), content: content.slice(0, 12_000), description: "GOLD — COPD diagnosis, staging, and management guideline" };
    } catch {
      return null;
    }
  },
