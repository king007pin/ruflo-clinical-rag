...
export const nfiCrawler: CrawlerDef = {
  ...
  async fetchArticle(url: string): Promise<CrawlerArticle | null> {
    try {
      ...
      if (isPdf) {
        const res = await siteFetch(url, { headers: { "User-Agent": UA }, timeoutMs: 90000 });
      ...
      const res = await siteFetch(url, { headers: { "User-Agent": UA, Accept: "text/html" }, timeoutMs: 20000 });
      if (!res.ok) return null;
