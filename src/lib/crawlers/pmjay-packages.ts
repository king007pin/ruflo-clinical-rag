...
const DELAY_MS = 1000;
const BASE = "https://pmjay.gov.in";
const UA = "MediqRAG/1.0 (clinical research; contact: admin@mediq.ai)";

export const pmjayPackagesCrawler: CrawlerDef = {
  id: "pmjay-packages",
  name: "PM-JAY — National Health Authority Benefit Packages",
  ...
  async fetchUrls(): Promise<string[]> {
    ...
    const seedPages = [
      `${BASE}/health-benefit-packages`,
      `${BASE}/pre-authorization-guidelines`,
      `${BASE}/guidelines`,
    ];
    ...
    const fallbackUrls = [
      `${BASE}/health-benefit-packages`,
      `${BASE}/pre-authorization-guidelines`,
      `${BASE}/guidelines/clinical-criteria`
    ];
  ...
  async fetchArticle(url: string): Promise<CrawlerArticle | null> {
    try {
      ...
      const title = (h1Match?.[1] ?? titleMatch?.[1] ?? "PM-JAY Health Benefit Package").replace(/&[a-z]+;/g, " ").trim();
      ...
      return {
        ...
        description: "National Health Authority PM-JAY health benefit package clinical pre-authorization criteria and policy guidelines",
      };
    } catch {
      return null;
    }
